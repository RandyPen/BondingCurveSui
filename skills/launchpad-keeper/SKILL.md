...
- Fees: `set_fee_params(curve_fee_bps ≤ 1000, curve_fee_platform_bps, lp_fee_platform_bps, migration_fee_bps ≤ 1000)` — live pools keep their launch-time snapshot. `migration_fee_bps` (default 500) is taken from the raised quote at migration; the CLMM seed stays gap-free (both legs shrink by the fee ratio, excess base burned).
