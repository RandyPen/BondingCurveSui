...
- **Creator first-buy tranches**: at most one time-locked and one market-cap-locked tranche per pool (each a singleton dynamic field on the `Pool`, so a launch cannot register the same lock kind twice), each with its own unlock condition:
