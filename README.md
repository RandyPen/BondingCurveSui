...
- a complete on-chain lifecycle (launch → bonding-curve trading → Cetus CLMM full-range migration → LP burn → fee sharing), 123 unit tests including real CLMM pool creation;
...
Typical customization points when forking: fee-split recipients (e.g. route the treasury share to a staking or buyback contract), additional tranche unlock-condition kinds in `pool.move` (time and market-cap are implemented; a third kind is a new request type plus a new optional `create_token` parameter, not a new discriminant value — see the tranche bullet below), quote whitelist policy, and the frontend/keeper layer. The contracts are chain-complete with no privileged off-chain component beyond the single recommended migration keeper.
...
- **Currency (BurnOnly)**: at launch, after minting the fixed supply (default 800M curve + 200M LP = 1B total, 9 decimals), `make_supply_burn_only` consumes the TreasuryCap. Anyone can then burn base coins via the shared `Currency<T>`; total supply stays on-chain queryable and minting is impossible. The **MetadataCap is claimed into the pool** (dynamic object field): the creator — and only the creator — can later update the coin's name/description/icon via `update_base_metadata` (symbol is immutable in coin_registry). Buyers should be aware metadata is creator-mutable.
- **Creator first-buy tranches**: exactly two lock kinds exist — time and market-cap — and **each can be used at most once per pool**. A tranche is a *singleton dynamic field* on the `Pool` (`TimeTrancheKey {}` / `TvlTrancheKey {}`, empty structs: the key type is the identity, there is no index). **The input signature is the constraint.** `create_token` takes one optional typed request per kind — `Option<TimeTrancheRequest>` and `Option<TvlTrancheRequest>` — so the legal inputs are exactly: no tranche, one time, one market-cap, or one of each, and a repeated kind, an unknown kind and a market cap in a timestamp's slot are all *unrepresentable* rather than runtime-rejected. (This replaced three parallel `tranche_*` vectors plus a lock-kind byte, whose arity, raggedness and kind-validity aborts are all gone; a first-buy that locked nothing is still an abort rather than a skip, since otherwise it would leave the key free and hand a future same-kind write a fresh full cap.) When both are supplied the **time leg buys first** — both spend against the same rising curve, so the order decides which leg gets the cheaper tokens, and the contract fixes it instead of taking it from the caller. Staggered vesting is not expressible — a single time tranche is a cliff, and a single market-cap tranche gates once and then vests linearly. Each has its own independent base cap (`max_time_base` / `max_tvl_base`), so the two stack without competing; an over-cap buy is clamped to the cap with the excess quote refunded. Unlock calls take no index — they address the singleton directly, so `unlock_tranche_time` on a pool with only a market-cap tranche aborts `ETrancheNotFound` ("wrong kind" is a typed absence, not a runtime check). Conditions:
...
- **Regulated-coin (honeypot) rejection — fail-closed, no keeper**: a `DenyCapV2<T>` can only be minted by `create_regulated_currency_v2(otw)`, which consumes `T`'s one-time witness. `pool::seal` consumes that same OTW via the **unregulated** constructor. Since an OTW is single-use, a given `T` can be sealed (→ `FactoryReceipt`, provably unregulated) **or** regulated (→ DenyCap, no receipt) — never both. `create_token` requires the receipt by value, so a regulated coin can never reach a pool. This is a type-level guarantee, which is why the launchpad creates the currency rather than accepting one: the earlier design inspected `coin_registry::is_regulated`, but that returns `false` for the `Unknown` state the legacy path always produces, so it was fail-open and needed an off-chain marking keeper. That keeper is obsolete (design note: `docs/coin-factory-fail-closed-design.md`).
...
The launchpad — not the coin author — creates the currency. That is what makes the regulated-coin guarantee structural instead of a keeper's responsibility (see below).

1. **tx1** — publish the coin package. It contains only the one-time witness type and a one-line `init` that hands the OTW to `pool::seal`, which creates the `Currency<Base>` through `coin_registry`'s **unregulated** constructor, fixes decimals at 9 (`STD_BASE_DECIMALS`, not a caller parameter), claims the MetadataCap, and returns a `FactoryReceipt<Base>` to the publisher. The coin package has no other logic: it cannot mint, cannot inject transfer hooks, and cannot obtain a `DenyCap`. Reference template: `template/agent_coin_template/`; for programmatic launches, patch the pre-compiled bytecode with `@mysten/move-bytecode-template`.
2. **tx2** — `sui::coin_registry::finalize_registration` (permissionless) shares the `Currency<Base>` that `seal` sent to the registry address `0xc`. (It cannot be shared and used mutably in the same transaction, hence the split from tx3. The shared object's id differs from the temporary one at `0xc`.)
3. **tx3** — `pool::create_token<Base, Quote>` (or `create_token_entry`; takes Cetus `GlobalConfig` + `Pools`), consuming the `FactoryReceipt` **by value**: validation, minting, **Cetus pool-key reservation (permission pair)**, MetadataCap custody (creator keeps update rights), supply → BurnOnly, virtual-reserve derivation, creator first-buy tranches, pool shared.

Only tx3 needs the creator's signature: the platform can run tx1 and tx2 and hand over the receipt, which is a plain owned object only its holder can consume. Creator attribution is automatic (`pool.creator = sender`).
...
- **`launchpad-launch`** — the full agent launch flow: bytecode-template coin package publishing (its `init` calls `seal`) → `finalize_registration` Currency sharing → `create_token` (tranche parameter encoding and constraints);
- **`launchpad-keeper`** — platform operations: the migration keeper, permissionless cranks (fee distribution / LP fees / rewards / unlocks), admin operations and an alerting checklist.
...
sui move test  --build-env mainnet   # 123 tests
```

`--build-env` is required: the Cetus dependencies are git-pinned per network
(see the comment in `Move.toml`), so a bare `sui move build` under a localnet
client environment cannot resolve them.
...
7. **Migration keeper** (the only one): the drain buy leaves the pool `COMPLETED`; migration is a separate permissionless call. Frontends should attach `migrate` to the draining buy in the same PTB (see `launchpad-trade`), which covers the common case — but that is best-effort, so run the keeper as the backstop: it must handle the buyer who did not attach `migrate` (aggregators, direct SDK calls), and the case where attaching it would have aborted the buyer's own transaction. Discover work by subscribing to `CurveCompletedEvent` for latency, and reconcile against the `LaunchpadConfig.pools` table for completeness — events age out of node indexes, that table does not.
